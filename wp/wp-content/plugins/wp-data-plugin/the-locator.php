					...
					if( $fields ) :
						...
						foreach( $fields as $field ) : ?>
							<tr href="<?php echo $field['id'] ?>" id="<?php echo $field['id'] ?>">
								...
								<td><a href="<?php echo $field['id'] ?>" class="edit button-primay">Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;
								<a href="<?php echo $field['id'] ?>" class="delete button-primay">Delete</a></td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
				</tbody>
...
<?php
	...
	function gurustu_find_location(){
		...
		$query = "SELECT id, name, address, city, state, zip, lat, lng, ( 3959 * acos( cos( radians( $lat ) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians( $lng ) ) + sin( radians( $lat ) ) * sin( radians( lat ) ) ) ) 
	...
	function show_gurustu_locator( $atts ){
		...
		<style type="text/css">
			...
			#locator_map_canvas div div.gmnoprint div span,#locator_map_canvas div div.gmnoprint div span a {
		...
		<form id="location_search_form" action="<?php echo get_permalink( $post->ID ) ?>" method="GET">
			...
			<input type="text" name="locator_zip" id="locator_zip" placeholder="Enter Zip/Postal Code" value="" />
			<input type="submit" id="gurustu_address_submit" value="Find a Food Bank" />
		</form>
	...
	function show_gurustu_locator_widget( $atts ){
		...
		<div id="locator_message_box"></div>
		...
		<div class="location_search_form_widget">
			<form id="location_search_form_widget" action="" method="GET">
				...
				<input type="submit" id="gurustu_address_submit_widget" value="Locate A Food Bank" />
			</form>
		</div>
		<br/>
	    <script type="text/javascript">
